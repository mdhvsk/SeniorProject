...
boolean accelerometerStart = false;
boolean firstRun = true;
unsigned long firstRunTime = 0;
...
// run motor
  ...
  // Any errors (hasn't been called on yet)
    ...
    if (accelerometerStart) {
    if (firstRun) {
      initialTime = millis();
      firstRun = false;
    }
    unsigned long currentMillis = millis() - initialTime;
    // Get the current time
    if ((micros() - lastMicros > INTERVAL) && currentMillis > 1000) {
